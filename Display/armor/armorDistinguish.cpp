...
#ifdef USE_BP
  // net = cv::dnn::readNetFromCaffe("../trainData/deploy.prototxt",
  // "../trainData/_iter_2000.caffemodel");
...
#ifdef USE_KCF
// kcf追踪器
void ArmorDistinguish::armorTrackerInit(const cv::Mat &src,
  ...
  if (_isTrackerStart) {
    ...
    if (!_tracker->update(_src, _trackerRect)) {
      ...
      // 越界则需要从全图搜索
      if ((_trackerRect & cv::Rect2d(0, 0, _imgWidth, _imgHeight)) !=
          _trackerRect) {
        _searchRect = cv::Rect2d(0, 0, _imgWidth, _imgHeight);
      }
      // 搜索区域需要扩大两倍
...
// 识别前的图片准备
void ArmorDistinguish::imagePreprocess(const cv::Mat &src,
                                       EnemyColor enemyColor) {

  _para.enemyColor = enemyColor;
  const cv::Point &lastResult =
      _resLast.center; // 上一个运行结果取上一次方框的中心
  if (lastResult.x == 0 ||
      lastResult.y == 0) { // 如果上一次结果的x或y为0，就以整个图片的大小作方框
    _src = src; // 继承原图片地址
    ...
    cv::Rect rect =
        _resLast.boundingRect(); // 返回包含旋转矩形的最小右上整数矩形。
    ...
    int x2 = MIN(int(lastResult.x + (rect.width * ratioToWidth)),
                 src.cols); // 计算结果不大于图片长宽
  ...
  if (enemyColor == ENEMY_RED) {
    ...
    cv::threshold(_separationSrc, _separationSrc, _para.separationThreshold_RED,
                  255, cv::THRESH_BINARY); // 红蓝二值化
    cv::threshold(_separationSrcGreen, _separationSrcGreen,
                  _para.separationThreshold_GREEN, 255,
                  cv::THRESH_BINARY); // 红绿二值化
    cv::dilate(_separationSrc, _separationSrc, Util::structuringElement3());
    cv::dilate(_separationSrcGreen, _separationSrcGreen,
               Util::structuringElement3()); // 膨胀

    _maxColor = _separationSrc & _graySrc & _separationSrcGreen &
                _separationSrcWhite; // 逻辑与获得最终二值化图像
    cv::dilate(_maxColor, _maxColor, Util::structuringElement3()); // 膨胀
    // cv::morphologyEx(_maxColor, _maxColor, cv::MORPH_OPEN,
    // Util::structuringElement3());
  } else {
    cv::threshold(splitSrc[2], _purpleSrc, _para.grayThreshold_PURPLE, 255,
                  cv::THRESH_BINARY); // 防止误识别紫色基地
    ...
    cv::threshold(_separationSrc, _separationSrc,
                  _para.separationThreshold_BLUE, 255, cv::THRESH_BINARY);
    cv::threshold(_separationSrcGreen, _separationSrcGreen,
                  _para.separationThreshold_GREEN, 255,
    ...
    _maxColor = _separationSrc & _graySrc & _separationSrcGreen &
                _separationSrcWhite & _purpleSrc; // 逻辑与获得最终二值化图像
    cv::dilate(_maxColor, _maxColor, Util::structuringElement3()); // 膨胀
    // cv::morphologyEx(_maxColor, _maxColor, cv::MORPH_OPEN,
    // Util::structuringElement3());
  }
...
void ArmorDistinguish::findLightBarContour() {
  // 找到灯条轮廓
  cv::findContours(_maxColor, allContours, hierarchy, cv::RETR_EXTERNAL,
                   cv::CHAIN_APPROX_SIMPLE);
  // 直接通过轮廓筛选  然后才通过拟合的旋转矩形筛选
  for (int i = 0; i < allContours.size(); ++i) {
    if (hierarchy[i][3] == -1) {
      cv::RotatedRect scanRect =
          cv::minAreaRect(allContours[i]); // 检测最小面积的矩形

      cv::Point2f vertices[4];
      scanRect.points(vertices);

      if (fabs(vertices[1].x - vertices[3].x) >
          fabs(vertices[1].y - vertices[3].y))
        continue;
      // std::cout << "scanRect.size.area():" << scanRect.size.area() <<
      // std::endl;
      ...
      if (longSide > _para.maxLightBarLength ||
          longSide < _para.minLightBarLength ||
          shortSide > _para.maxLightBarWidth ||
          shortSide < _para.minLightBarWidth)
        continue;
...
void ArmorDistinguish::lightBarFilter() {
  ...
  for (size_t i = 0; i < possibleRects.size() - 1; ++i) {
    for (size_t j = i + 1; j < possibleRects.size(); ++j) {
      float maxLength_1 = 0, maxLength_2 = 0, maxLength_between = 0,
            minLength_between;
      ...
      if (fabs(possibleRects[i].center.x - possibleRects[j].center.x) >
              maxLength_between ||
          fabs(possibleRects[i].center.y - possibleRects[j].center.y) >
              minLength_between * 0.3f) {
        // 最长的一边也必须在一定的长度范围内,面积也要在一定范围内
        float heightScale = maxLength_1 > maxLength_2
                                ? (maxLength_1 / maxLength_2)
                                : (maxLength_2 / maxLength_1);
        if (heightScale < 1.4f) {
          centre_between = sqrtf(
              (float)pow(possibleRects[i].center.x - possibleRects[j].center.x,
                         2) +
              (float)pow(possibleRects[i].center.y - possibleRects[j].center.y,
                         2));
          // 限制装甲矩形的长宽比
          if (centre_between < maxLength_between * scale_max) {
            ...
            if (fabs(vertices1[0].x - vertices1[2].x) >
                fabs(vertices1[1].x - vertices1[3].x)) {
              if (vertices1[0].y > vertices1[2].y) {
                tan1 = (vertices1[0].y - vertices1[2].y) / vertices1[0].x -
                       vertices1[2].x;
              } else {
                tan1 = (vertices1[2].y - vertices1[0].y) / vertices1[2].x -
                       vertices1[0].x;
              }
            } else {
              if (vertices1[1].y > vertices1[3].y) {
                tan1 = (vertices1[1].y - vertices1[3].y) / vertices1[1].x -
                       vertices1[3].x;
              } else {
                tan1 = (vertices1[3].y - vertices1[1].y) / vertices1[3].x -
                       vertices1[1].x;
              }
            }

            if (fabs((vertices2[0].y - vertices2[2].y) /
                     (vertices2[0].x - vertices2[2].x)) >
                fabs((vertices2[1].y - vertices2[3].y) /
                     (vertices2[1].x - vertices2[3].x))) {
              if (vertices2[0].y > vertices2[2].y) {
                tan2 = (vertices2[0].y - vertices2[2].y) / vertices2[0].x -
                       vertices2[2].x;
              } else {
                tan2 = (vertices2[2].y - vertices2[0].y) / vertices2[2].x -
                       vertices2[0].x;
              }
            } else {
              if (vertices2[1].y > vertices2[3].y) {
                tan2 = (vertices2[1].y - vertices2[3].y) / vertices2[1].x -
                       vertices2[3].x;
              } else {
                tan2 = (vertices2[3].y - vertices2[1].y) / vertices2[3].x -
                       vertices2[1].x;
              }
            }

            float lightAngle =
                Util::lineToLineAngle(shortCenter1[0], shortCenter1[1],
                                      shortCenter2[0], shortCenter2[1]);
            ...
            if (calSkewingAngle(possibleRects[i], possibleRects[j],
                                shortCenter1) < 75.0f ||
                calSkewingAngle(possibleRects[i], possibleRects[j],
                                shortCenter2) < 75.0f)
              continue;

            // std::cout << "lineToLineAngle<13:" <<
            // Util::lineToLineAngle(shortCenter1[0], shortCenter1[1],
            // shortCenter2[0], shortCenter2[1]) << std::endl; std::cout <<
            // "calSkewingAngle<75:" << calSkewingAngle(inputRects[i],
            // inputRects[j], shortCenter1) << std::endl;

            cv::RotatedRect armor =
                Util::boundingRRect(possibleRects[i], possibleRects[j]);
            if (fabs(armor.angle) > 10.0f && fabs(armor.angle) < 170.0f)
              continue;

            ArmorStruct armorStruct;
            armorStruct.lightAngle = lightAngle;
            armorStruct.partLightBars[0] =
                (possibleRects[i].center.x < possibleRects[j].center.x
                     ? possibleRects[i]
                     : possibleRects[j]);
            armorStruct.partLightBars[1] =
                (possibleRects[i].center.x > possibleRects[j].center.x
                     ? possibleRects[i]
                     : possibleRects[j]);
...
void ArmorDistinguish::getShortCenter(cv::RotatedRect rect,
                                      cv::Point2f *shortCenter) {
  if (rect.size.width <= 0) {
    return;
  }
  cv::Point2f verts[4];
  rect.points(verts);
  if (Util::pointDistance(verts[0], verts[1]) <
      Util::pointDistance(verts[1], verts[2])) {
...
float ArmorDistinguish::calSkewingAngle(cv::RotatedRect rect1,
                                        cv::RotatedRect rect2,
                                        cv::Point2f *shortCenter) {
  if (rect1.size.width <= 0 || rect1.size.height <= 0) {
    return float();
  }
  return Util::lineToLineAngle(rect1.center, rect2.center, shortCenter[0],
                               shortCenter[1]);
}
...
void ArmorDistinguish::distinguishArmorType(ArmorStruct &armorStructs) {
  float longSide_1 =
      Util::findExtremumOfSide(armorStructs.partLightBars[0], LONG_SIDE);
  float longSide_2 =
      Util::findExtremumOfSide(armorStructs.partLightBars[1], LONG_SIDE);
  float heightScale = longSide_1 > longSide_2 ? (longSide_1 / longSide_2)
                                              : (longSide_2 / longSide_1);

  // std::cout << "getRectLengthWidthRatio:" <<
  // Util::getRectLengthWidthRatio(armorStructs.armorRect) << std::endl;
...
void ArmorDistinguish::findNearestArmor(std::vector<ArmorStruct> &armorStructs,
...
void ArmorDistinguish::removeWrongArmor() {
  ...
  for (int i = 0; i < armorStructs.size(); ++i) {
    for (int j = 0; j < possibleRects.size(); ++j) {
      centreDistance[0] = Util::pointDistance(
          armorStructs[i].partLightBars[1].center, possibleRects[j].center);
      centreDistance[1] = Util::pointDistance(
          armorStructs[i].partLightBars[0].center, possibleRects[j].center);
  ...
  for (int i = 0; i < armorStructs.size() - 1; ++i) {
    for (int j = i + 1; j < armorStructs.size(); ++j) {
      centreDistance[0] =
          Util::pointDistance(armorStructs[i].partLightBars[1].center,
                              armorStructs[j].partLightBars[0].center);
      centreDistance[1] =
          Util::pointDistance(armorStructs[i].partLightBars[0].center,
                              armorStructs[j].partLightBars[1].center);
      if (!centreDistance[0]) {

        if (armorStructs[i].armorType == true &&
            armorStructs[j].armorType == true) {
          if (armorStructs[i].lightAngle > armorStructs[j].lightAngle) {
            ...
            if (armorStructs[i].armorRect.size.width /
                    armorStructs[i].armorRect.size.height >
                armorStructs[j].armorRect.size.width /
                    armorStructs[j].armorRect.size.height) {
              armorStructs.erase(std::begin(armorStructs) + i);
            } else {
              armorStructs.erase(std::begin(armorStructs) + j);
            }
...
// 根据权重找装甲板
void ArmorDistinguish::findArmorByWeight(
    std::vector<ArmorStruct> &armorStructs, cv::RotatedRect &resultRect,
    ArmorRectHistoricalDataList &armorRectData) {
  // float aver = 0.0f;
  for (size_t i = 0; i < armorStructs.size(); ++i) {
    float rectsArea = armorStructs[i].armorRect.size.height *
                      armorStructs[i].armorRect.size.width;
    armorRectData.areaChangeRate.push_back(
        fabs(rectsArea - _armorRectHistoricalData.area.back()) / rectsArea);
    armorRectData.areaChangeRateChangeRate.push_back(
        fabs(armorRectData.areaChangeRate.back() -
             _armorRectHistoricalData.areaChangeRate.back()) /
        rectsArea);
    float rectsLengthWidthRatio =
        Util::getRectLengthWidthRatio(armorStructs[i].armorRect);
    armorRectData.lenWidChangeRate.push_back(
        fabs(rectsLengthWidthRatio -
             _armorRectHistoricalData.lengthWidthRatio.back()));
    cv::Point2f nowCenter2D =
        cv::Point2f(armorStructs[i].armorRect.center.x + _restoreRect.x,
                    armorStructs[i].armorRect.center.y + _restoreRect.y);
    armorRectData.xChangeRate.push_back(
        fabsf(nowCenter2D.x - _armorRectHistoricalData.x.back()) / rectsArea);
    armorRectData.yChangeRate.push_back(
        fabsf(nowCenter2D.y - _armorRectHistoricalData.y.back()) / rectsArea);
  }
  maxWeightNum = findVarianceMaxWeight(_armorRectHistoricalData, armorRectData,
                                       _maxWeightValue);
...
void ArmorDistinguish::armorChooseTarget(EnemyColor enemyColor) {
  ...
  while (it != armorStructs.end()) {
    ...
    if (!getBlobColor(enemyColor, (*it).partLightBars[0],
                      (*it).partLightBars[1])) {
    ...
    if (_carType == NEW_SENTRY_BELOW || _carType == NEW_SENTRY_ABOVE) {
      ...
      if (bounding_roi.width < bounding_roi.height) {
        cv::Point centre((bounding_roi.tl().x + bounding_roi.br().x) / 2,
                         (bounding_roi.tl().y + bounding_roi.br().y) / 2);
      ...
      cv::Point2f new_center =
          rects.center - cv::Point2f((float)bounding_roi.x,
                                     (float)bounding_roi.y); // 计算新的中心点
      cv::Mat roi_src = _src(bounding_roi);
      cv::Mat rotation =
          cv::getRotationMatrix2D(new_center, rects.angle, 1); // 得到旋转后的图
      cv::Mat rectify_target;
      cv::warpAffine(roi_src, rectify_target, rotation,
                     bounding_roi.size()); // 做一次仿射变换

      cv::waitKey(1);

      int minInsert =
          MIN(rectify_target.rows,
              rectify_target.cols); // 选最小的一边，圈一个正方形的ROI
      cv::Point tl(rectify_target.cols / 2 - minInsert / 4,
                   rectify_target.rows / 2 - minInsert / 4);
      cv::Point br(rectify_target.cols / 2 + minInsert / 4,
                   rectify_target.rows / 2 + minInsert / 4);
      cv::Rect square = cv::Rect(tl, br);
      cv::Mat square_target = rectify_target(square);
      cv::Mat resizeImg;
      cv::resize(square_target, resizeImg, cv::Size(200, 200), 0,
                 0);                                       // 重新装载大小
      cv::cvtColor(resizeImg, _bpImg, cv::COLOR_BGR2GRAY); // 前哨站空白区域
      cv::threshold(_bpImg, _bpImg, 0, 255,
                    cv::THRESH_OTSU); // 识别2号装甲板二值化
...
#if USE_CAFFE
    // Mat inputBlob = dnn::blobFromImage(resizeImg, 0.00390625f, Size(20, 20),
    // Scalar(), false);
    Mat inputBlob =
        dnn::blobFromImage(resizeImg, 1, Size(20, 20), Scalar(), false);
    ...
    //        LOG::info("case of armor: " + to_string(classId) + " accuracy: " +
    //        to_string(classProb * 100));
    ...
    //}
    //(*it).carId = response;
    // std::string strttt[] = { "0","hero
    // ","engineer","infantry3","infantry4","5","base and skirmish","sentinel"
...
bool ArmorDistinguish::getBlobColor(EnemyColor enemyColor,
                                    const cv::RotatedRect &blobPos1,
                                    const cv::RotatedRect &blobPos2) {
...
bool ArmorDistinguish::embeddedRectJudge(cv::RotatedRect r1,
                                         cv::RotatedRect r2) {
  double distance =
      r1.size.width > r1.size.height ? r1.size.width / 2 : r1.size.height / 2;
...
int ArmorDistinguish::armorHogSvm(cv::Mat &inputMat) {
  ...
  cv::HOGDescriptor myHog = cv::HOGDescriptor(
      matSize, cv::Size(16, 16), cv::Size(8, 8), cv::Size(8, 8), 9);
...
// 装甲板协方差权重计算
float ArmorDistinguish::armorVarianceWeightCalculate(
    ArmorRectHistoricalDataList &data, size_t i) {
  return (1 - (data.areaChangeRateChangeRateVarianceList[i] *
                   _armorWeightCalculateCoefficient.areaVarianceCoefficient +
               data.xChangeRateVarianceList[i] *
                   _armorWeightCalculateCoefficient.xVarianceCoefficient +
               data.yChangeRateVarianceList[i] *
                   _armorWeightCalculateCoefficient.yVarianceCoefficient +
               data.lenWidChangeRateVarianceList[i] *
                   _armorWeightCalculateCoefficient.lenWidVarianceCoefficient));
}

// 找到权重最大的装甲板
int ArmorDistinguish::findVarianceMaxWeight(
    ArmorRectHistoricalDataList &lastArmorRectData,
    struct ArmorRectHistoricalDataList &nowArmorRectData, float &weight) {
  int j = 0;
...
// 历史装甲板数据重置
void ArmorDistinguish::armorRectListDataReset(
    struct ArmorRectHistoricalDataList &armorRectData) {
...
// 历史装甲板基础数据装填
void ArmorDistinguish::armorRectListBaseDataIndex(
    struct ArmorRectHistoricalDataList &armorRectData, cv::RotatedRect &rect) {
  cv::Point2f resultCenter = cv::Point2f(rect.center.x + _restoreRect.x,
                                         rect.center.y + _restoreRect.y);
  if (armorRectData.area.size() < FIFO_LEN) {
    // 填充新数据
    armorRectData.area.push_back(rect.size.height * rect.size.width);
    armorRectData.lengthWidthRatio.push_back(
        Util::getRectLengthWidthRatio(rect));
    ...
    armorRectData.lengthWidthRatio.erase(
        std::begin(armorRectData.lengthWidthRatio));
...
// 历史装甲板偏差数据装填
void ArmorDistinguish::armorRectListBiasDataIndex(
    struct ArmorRectHistoricalDataList &lastArmorRectData,
    struct ArmorRectHistoricalDataList &nowArmorRectData, uint8_t i,
    uint8_t mode) {

  if (lastArmorRectData.areaChangeRate.size() < FIFO_LEN) {
    // 填充新数据
    lastArmorRectData.areaChangeRate.push_back(
        nowArmorRectData.areaChangeRate[i]);
    lastArmorRectData.areaChangeRateChangeRate.push_back(
        nowArmorRectData.areaChangeRateChangeRate[i]);
    lastArmorRectData.lenWidChangeRate.push_back(
        nowArmorRectData.lenWidChangeRate[i]);
    ...
    // 擦除第一个数据
    lastArmorRectData.areaChangeRate.erase(
        std::begin(lastArmorRectData.areaChangeRate));
    lastArmorRectData.areaChangeRateChangeRate.erase(
        std::begin(lastArmorRectData.areaChangeRateChangeRate));
    lastArmorRectData.lenWidChangeRate.erase(
        std::begin(lastArmorRectData.lenWidChangeRate));
    lastArmorRectData.xChangeRate.erase(
        std::begin(lastArmorRectData.xChangeRate));
    lastArmorRectData.yChangeRate.erase(
        std::begin(lastArmorRectData.yChangeRate));
    // 填充新数据
    lastArmorRectData.areaChangeRate.push_back(
        nowArmorRectData.areaChangeRate[i]);
    lastArmorRectData.areaChangeRateChangeRate.push_back(
...
// 装甲板方差数据装填
void ArmorDistinguish::armorRectListVarianceDataCalculate(
    struct ArmorRectHistoricalDataList &lastArmorRectData,
    struct ArmorRectHistoricalDataList &nowArmorRectData, size_t i) {
  // 填充新数据
  lastArmorRectData.areaChangeRate.back() = nowArmorRectData.areaChangeRate[i];
  lastArmorRectData.lenWidChangeRate.back() =
      nowArmorRectData.lenWidChangeRate[i];
  ...
  nowArmorRectData.areaChangeRateChangeRateVarianceList.push_back(
      Util::varianceDataCalculate(lastArmorRectData.areaChangeRate));
  nowArmorRectData.lenWidChangeRateVarianceList.push_back(
      Util::varianceDataCalculate(lastArmorRectData.lenWidChangeRate));
  nowArmorRectData.xChangeRateVarianceList.push_back(
      Util::varianceDataCalculate(lastArmorRectData.xChangeRate));
  nowArmorRectData.yChangeRateVarianceList.push_back(
      Util::varianceDataCalculate(lastArmorRectData.yChangeRate));
}

std::vector<ArmorDistinguish::ArmorStruct>
ArmorDistinguish::getAttackOrder(std::vector<ArmorStruct> &armorStructs) {
  if (armorStructs.empty()) {
    ...
    for (size_t i = 0; i < armorStructs.size(); ++i) {
      if (armorStructs[i].carId == 1 && orderFlag[0] == false) {
        orderFlag[0] = true;
      } else if ((armorStructs[i].carId == 3 || armorStructs[i].carId == 4) &&
                 orderFlag[1] == false) {
        orderFlag[1] = true;
...
void ArmorDistinguish::topProcess(float yawAngle) {
  ...
  if (_topStatusFlag == false) {
    ...
    // 计算1->0->.....->0->1->1周期时间间隔
    else if (_findsameFlag == true && _sameTargetFlag == true &&
             _lastsameTargetFlag == true) {
      _oneCycletime =
          (double)((cv::getTickCount() - _topTime) / cv::getTickFrequency());
      _losesameFlag = false;
      _findsameFlag = false;
      // std::cout << "oneCycletime:" << _oneCycletime << endl;
    }
    // 上次sameTargetFlag为true而现在为false，则认为寻找下一块装甲板
    else if (_losesameFlag == false && _lastsameTargetFlag == true &&
...
cv::RotatedRect ArmorDistinguish::getResult() {
...
#endif // USE_BP
#ifdef USE_WEIGHT
  _timeBias = (double)((cv::getTickCount() - _lastTime) /
                       cv::getTickFrequency()); // 目标捕捉时间偏差
  if (_timeBias > 3.0) {
    _sameTargetFlag = false;
  } // 两次目标捕捉时间过长
#ifdef DEBUG_WEIGHT
  ...
  if (_armorRectHistoricalData.area.size() < 1 || !_sameTargetFlag) {
    ...
    armorRectListBaseDataIndex(_armorRectHistoricalData,
                               resultRect_); // 装甲板基本数据装填
    _otherTargetCount = 0;                   // 跟随错误计数清零
    _sameTargetFlag = true;                  // 确定跟随目标
  } else {
    struct ArmorRectHistoricalDataList
        armorRectData; // 有历史装甲板数据权重判断装甲板做跟随目标
    findArmorByWeight(armorStructs, resultRect_, armorRectData);
    if (_maxWeightValue >
            _armorWeightCalculateCoefficient.sameTargetThreshold &&
        _nowCarID == _lastCarID) { // 权重阈值存取最大权重目标
      _otherTargetCount = 0;
      armorRectListBaseDataIndex(_armorRectHistoricalData,
                                 resultRect_); // 装甲板基本数据装填
      armorRectListBiasDataIndex(_armorRectHistoricalData, armorRectData,
                                 maxWeightNum, 0); // 装甲板偏差数据装填
    } else {
      ...
      if (_otherTargetCount > OTHER_TARGET_COUNT_LIMIT) {
        ...
        cout << "area:  "
             << armorRectData
                        .areaChangeRateChangeRateVarianceList[maxWeightNum] *
                    _armorWeightCalculateCoefficient.areaVarianceCoefficient
             << endl;
        cout << "x:  "
             << armorRectData.xChangeRateVarianceList[maxWeightNum] *
                    _armorWeightCalculateCoefficient.xVarianceCoefficient
             << endl;
        cout << "y:  "
             << armorRectData.yChangeRateVarianceList[maxWeightNum] *
                    _armorWeightCalculateCoefficient.yVarianceCoefficient
             << endl;
        cout << "lw:  "
             << armorRectData.lenWidChangeRateVarianceList[maxWeightNum] *
                    _armorWeightCalculateCoefficient.lenWidVarianceCoefficient
             << endl;
...
// 识别过程函数
cv::RotatedRect ArmorDistinguish::process(const cv::Mat &src,
                                          EnemyColor enemyColor,
                                          CarType carType, bool isReset,
                                          DistinguishMode distinguishMode,
                                          float yawAngle, bool topRest) {
  if (isReset) {
    _restoreRect = cv::Rect(0, 0, src.cols, src.rows);
  }
  ...
  if (_resultRect.size.width > 0) {
    ...
    if (_isReulstFind) {
      float lightBarWidthL =
          _leftLightBar.size.width < _leftLightBar.size.height
              ? _leftLightBar.size.width
              : _leftLightBar.size.height;
      float lightBarHeightL =
          _leftLightBar.size.width > _leftLightBar.size.height
              ? _leftLightBar.size.width
              : _leftLightBar.size.height;
      float lightBarWidthR =
          _rightLightBar.size.width < _rightLightBar.size.height
              ? _rightLightBar.size.width
              : _rightLightBar.size.height;
      float lightBarHeightR =
          _rightLightBar.size.width > _rightLightBar.size.height
              ? _rightLightBar.size.width
              : _rightLightBar.size.height;

      cv::Point2d recttl =
          cv::Point2d((double)(_leftLightBar.center.x - 2.0 * lightBarWidthL),
                      (double)(_leftLightBar.center.y - 1.5 * lightBarHeightL));
      cv::Point2d rectbr = cv::Point2d(
          (double)(_rightLightBar.center.x + 2.0 * lightBarWidthR),
          (double)(_rightLightBar.center.y + 1.5 * lightBarHeightR));
    ...
    resultRect.center +=
        cv::Point2f((float)_searchRect.x, (float)_searchRect.y);
#else
    _resultRect.center +=
        cv::Point2f((float)_restoreRect.x, (float)_restoreRect.y);
#endif
    ...
    ++_lost_cnt;
    // 丢失目标后逐步扩大搜索范围
    if (carType == NEW_SENTRY_ABOVE || carType == NEW_SENTRY_BELOW ||
        carType == OLD_SENTRY_ABOVE || carType == OLD_SENTRY_BELOW) {
      if (_lost_cnt < 3)
        _resLast.size = cv::Size2f((float)(_resLast.size.width * 3),
                                   (float)(_resLast.size.height * 3));
      else if (_lost_cnt == 6)
        _resLast.size = cv::Size2f((float)(_resLast.size.width * 1.5),
                                   (float)(_resLast.size.height * 1.5));
      else if (_lost_cnt == 9)
        _resLast.size = cv::Size2f((float)(_resLast.size.width * 1.5),
                                   (float)(_resLast.size.height * 1.5));
  ...
  // std::cout << "_resultRect.center.x:" << _resultRect.center.x << std::endl
  // << std::endl;
  return _resultRect;
}
